...
**[.github/workflows/ci.yml](.github/workflows/ci.yml)** - CI/CD pipeline (optimized with single dependency installation)
**[.github/actions/setup-build/action.yml](.github/actions/setup-build/action.yml)** - Reusable setup composite action

### Pipeline Architecture

The refactored CI/CD pipeline uses a **single dependency installation strategy** with optimized caching and parallel job execution.

**Job Dependency Graph:**

```yaml
setup-build (installs dependencies once, caches node_modules)
  ├─> lint-format (parallel: lint + format checks)
  ├─> test (parallel: Node 24.x, 25.x matrix)
  └─> build-apps (builds both web and Electron Vue apps)
        ├─> deploy-pages (GitHub Pages deployment)
        └─> build-platforms (orchestrates platform builds)
              ├─> build-mac (macOS DMG + ZIP)
              ├─> build-windows (Windows EXE + ZIP)
...
2. **Optimized Caching**: Hash-based cache keys for node_modules (`package-lock.json` hash)
3. **Parallel Execution**: Lint/format and test jobs run in parallel
4. **Build Reuse**: Both web and Electron builds created in single `build-apps` job
5. **Artifact Reuse**: Platform builds download pre-built Vue app artifacts
